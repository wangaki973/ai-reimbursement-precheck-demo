...
2. 不覆盖全部费用类型，仅覆盖 Demo 核心场景。
3. 不接入真实 ERP，仅使用可替换的模拟工具接口。
4. 不包含 OCR/票据识别能力，不处理图片或 PDF 抽取。

### 3.3 Demo 前置假设与输入边界（无 OCR）
本 Demo 明确以前端已成功获取并传入结构化发票 JSON 为起点。Agent 负责“规则判断、缺失追问、预算校验、结论输出”，不负责票据图片/PDF 的 OCR 识别与字段抽取。
